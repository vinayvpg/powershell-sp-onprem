[CmdletBinding()]
param(
    [Parameter(Mandatory=$true, Position=0, HelpMessage="Root scope web application url whose users should be migrated")]
    [string] $webAppUrl,
...
Write-Host "Processing site '$webAppUrl'..." -ForegroundColor Cyan
<#
...
"UserType `t DisplayName `t CurrentLogin `t NewLogin `t WebUrl `t Moved?" | out-file $reportCSVPath
...
{ 
    ...
    {
        Write-Host "-----------------------------------------------------------------------------------------------"
        Write-Host "Processing site collection $($siteColl.url)...." -BackgroundColor DarkYellow
        
        foreach($web in $siteColl.AllWebs) 
        { 
            $webUrl = $web.url
            Write-Host "Processing web...$webUrl" -BackgroundColor DarkGreen
            ...
            foreach($user in $users)
            {
                ...
                {#>
                    ...
                    "$userType `t $userDisplayName `t $userLogin `t $username `t $webUrl `t $moved" | out-file $reportCSVPath -Append
                }
            #}
        }
