<#
.Description  
    Performs an inventory of web applications, farm solutions, site collections, webs, templates, features, lists, content types, site columns, permissions, webparts and workflows
    Generates a csv for each artifact type
         
.Parameter - DestinationFolder  
    Full path to the folder that will hold the csv files
.Parameter - LogFilePrefix 
    Prefix to associate with each csv file
.Parameter - ClearPriorLogs 
    Switch parameter to clear out existing csv files

.Usage 
    Perform a full inventory with custom report destination and csv file prefix
     
    PS >  Get-SPFarmInventory.ps1 -DestinationFolder "e:\temp\inventory" -LogFilePrefix "MOC_"

.Usage 
    Perform a full inventory with default parameters and clear out previous logs
     
    PS >  Get-SPFarmInventory.ps1 -ClearPriorLogs
#>
[CmdletBinding()]
param(
    [Parameter(Mandatory=$false, Position=0, HelpMessage="Full path to the folder that will hold the csv files")]
    [string] $DestinationFolder="E:\Scripts\Infrastructure\Inventory",

    [Parameter(Mandatory=$false, Position=1, HelpMessage="Prefix to associate with each csv file")]
    [string] $LogFilePrefix = "MOC_PROD_",

    [Parameter(Mandatory=$false, Position=2, HelpMessage="Should existing report files be cleared out?")]
    [switch] $ClearPriorLogs = $true
)
...
$defaultReportDir = "$([IO.Path]::GetDirectoryName($myInvocation.MyCommand.Definition))\InventoryReport"

if([string]::IsNullOrWhiteSpace($DestinationFolder)) {
    if(-not (Test-Path $defaultReportDir -PathType Container)) {
        Write-Host "Destination folder was not specified. Creating inventory report folder....$defaultReportDir"
        md -Path $defaultReportDir
    }

    $DestinationFolder = $defaultReportDir
}

Run-FullInventory $LogFilePrefix $DestinationFolder $ClearPriorLogs
